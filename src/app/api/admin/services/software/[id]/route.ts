...
export async function GET(
  ...
  try {
    ...
    const service = await SoftwareService.findById(id).lean();
...
export async function PUT(
  ...
  try {
    ...
    const service = await SoftwareService.findByIdAndUpdate(id, { ...body, updatedAt: new Date() }, { new: true, runValidators: true });
    if (!service) return NextResponse.json({ success: false, error: 'Service not found' }, { status: 404 });
...
export async function DELETE(
  ...
  try {
    ...
    const service = await SoftwareService.findByIdAndDelete(id);
    if (!service) return NextResponse.json({ success: false, error: 'Service not found' }, { status: 404 });
